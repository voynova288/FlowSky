...
test("api health, web root, and chat routes", async () => {
  await withServer(async (baseUrl) => {
    ...
    assert.match(await root.text(), /FlowSky \/ 流空/);
    ...
    const chat = await fetch(`${baseUrl}/chat`, {
      ...
      body: JSON.stringify({ user_id: "u1", session_id: "s1", input: { type: "text", text: "今天有点累" } }),
    });
...
test("api settings and memory routes", async () => {
  await withServer(async (baseUrl) => {
    const settings = await fetch(`${baseUrl}/settings?user_id=u1`, {
      method: "PATCH",
      headers: { "content-type": "application/json" },
    ...
    await fetch(`${baseUrl}/chat`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ user_id: "u1", session_id: "s1", input: { type: "text", text: "请记住我喜欢短句回复。" } }),
    });
    const memories = await fetch(`${baseUrl}/memories?user_id=u1`);
